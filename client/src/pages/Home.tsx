...
export default function Home() {
  ...
  return (
    <div className="overflow-hidden bg-[#f8f5ed] pb-20 text-[#283121] md:pb-0">
      ...
      <section id="duvidas" className="bg-[#f8f5ed] py-22 sm:py-28" style={{ paddingTop: "42px", paddingBottom: "43px" }}><div className="container grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:gap-20"><div><p className="section-kicker">Perguntas frequentes</p><h2 className="mt-6 font-display text-4xl leading-[.98] tracking-[-.035em] text-[#2c3726] sm:text-5xl">Tudo o que vocês precisam saber antes de começar.</h2><p className="mt-6 text-base leading-7 text-[#5c6654]">Transparência também faz parte de um planejamento inteligente.</p></div><div className="divide-y divide-[#d9cfb9] border-y border-[#d9cfb9]">{faqs.map(([question, answer], index) => <div key={question}><button onClick={() => setOpenFaq(openFaq === index ? null : index)} className="flex w-full items-center justify-between gap-6 py-5 text-left" aria-expanded={openFaq === index}><span className="text-sm font-extrabold leading-6 text-[#3b4932] sm:text-[0.95rem]">{question}</span><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#c5ad78] text-[#836432] transition ${openFaq === index ? "rotate-180 bg-[#ead8a9]" : ""}`}><ChevronDown className="h-4 w-4" /></span></button>{openFaq === index && <p className="max-w-2xl pb-6 pr-10 text-sm leading-6 text-[#65705b]">{answer}</p>}</div>)}</div></div></section>
