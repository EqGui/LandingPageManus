...
export default function Home() {
  ...
  return (
    <div className="overflow-hidden bg-[#f8f5ed] pb-20 text-[#283121] md:pb-0">
      ...
      <section id="objecoes" className="bg-[#fbf8f0] py-22 sm:py-28"><div className="container grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:gap-20"><div><h2 className="mt-6 font-display text-4xl leading-[.98] tracking-[-.035em] text-[#2c3726] sm:text-5xl">Talvez você esteja pensando...</h2><p className="mt-6 text-base leading-7 text-[#5d6754]">Ter dúvidas antes de investir em um método é esperado. Veja como ele se encaixa em diferentes momentos de planejamento.</p></div><div><div className="divide-y divide-[#d9cfb9] border-y border-[#d9cfb9]">{objections.map(([question, answer], index) => <div key={question}><button onClick={() => setOpenObjection(openObjection === index ? null : index)} className="flex w-full items-center justify-between gap-6 py-5 text-left"><span className="font-display text-xl text-[#3d4b34]">{question}</span><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#c5ad78] text-[#836432] transition ${openObjection === index ? "rotate-180 bg-[#ead8a9]" : ""}`}><ChevronDown className="h-4 w-4" /></span></button>{openObjection === index && <p className="max-w-2xl pb-6 pr-10 text-sm leading-6 text-[#65705b]">{answer}</p>}</div>)}</div><div className="mt-7 flex flex-col items-center gap-3 border-t border-[#d9cfb9] pt-6 text-center"><p className="max-w-md font-display text-lg italic leading-6 text-[#536047]">Clareza para decidir também é uma forma de cuidado.</p><a href={CHECKOUT_URL} className="btn-primary shrink-0">Quero o Método CASAR+ <ArrowRight className="h-4 w-4" /></a></div></div></div></section>
