...
const offerIncludes = [
  ["Curso completo Método CASAR+", "Para aplicar o processo de decisão em cada contratação."],
  ["Planner Financeiro CASAR+", "Para organizar orçamento, prioridades e pagamentos."],
  ["Radar CI para fornecedores", "Para comparar escopo, condições e pontos de atenção."],
  ["Guia de negociação e checklists", "Para preparar conversas e não esquecer etapas importantes."],
  ["Calculadora de orçamento", "Para estimar e distribuir o orçamento antes de negociar."],
];

const objections = [
  ["“Já contratei alguns fornecedores.”", "Tudo bem. O método também ajuda a revisar o orçamento restante e a aplicar critérios nas próximas contratações."],
  ["“Meu casamento não tem um orçamento alto.”", "Quanto menor ou maior o orçamento, mais importante é saber onde o dinheiro está sendo direcionado e como cada contratação impacta o total."],
  ["“Meu casamento é pequeno ou grande. Isso serve para mim?”", "Sim. Os critérios e processos de decisão podem ser aplicados independentemente do tamanho da celebração."],
  ["“Eu não sei negociar.”", "Você não precisa ser um negociador profissional. O método ensina o que analisar, o que perguntar e como estruturar uma conversa com fornecedores."],
  ["“Eu já tenho assessoria.”", "O Método CASAR+ não substitui uma assessoria. Ele ajuda o casal a participar melhor das decisões financeiras e das contratações."],
  ["“Meu casamento é daqui a poucos meses.”", "Ainda pode ser útil para revisar decisões, comparar propostas e organizar melhor as próximas contratações."],
];
...
export default function Home() {
  ...
  return (
    <div className="overflow-hidden bg-[#f8f5ed] pb-20 text-[#283121] md:pb-0">
      ...
      <section id="objecoes" className="bg-[#fbf8f0] py-22 sm:py-28"><div className="container grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:gap-20"><div><p className="section-kicker">Objeções comuns</p><h2 className="mt-6 font-display text-4xl leading-[.98] tracking-[-.035em] text-[#2c3726] sm:text-5xl">Talvez você esteja pensando...</h2><p className="mt-6 text-base leading-7 text-[#5d6754]">Ter dúvidas antes de investir em um método é esperado. Veja como ele se encaixa em diferentes momentos de planejamento.</p></div><div><div className="divide-y divide-[#d9cfb9] border-y border-[#d9cfb9]">{objections.map(([question, answer], index) => <div key={question}><button onClick={() => setOpenObjection(openObjection === index ? null : index)} className="flex w-full items-center justify-between gap-6 py-5 text-left"><span className="font-display text-xl text-[#3d4b34]">{question}</span><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#c5ad78] text-[#836432] transition ${openObjection === index ? "rotate-180 bg-[#ead8a9]" : ""}`}><ChevronDown className="h-4 w-4" /></span></button>{openObjection === index && <p className="max-w-2xl pb-6 pr-10 text-sm leading-6 text-[#65705b]">{answer}</p>}</div>)}</div><div className="mt-7 flex items-center justify-between gap-4 border-t border-[#d9cfb9] pt-6"><p className="font-display text-lg italic text-[#536047]">Clareza para decidir também é uma forma de cuidado.</p><a href="#oferta" className="btn-primary shrink-0">Quero o Método CASAR+ <ArrowRight className="h-4 w-4" /></a></div></div></div></section>

      <section id="oferta" className="relative isolate overflow-hidden bg-[#283121] py-22 sm:py-28">
        ...
        <div className="container">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-7 font-display text-4xl leading-[.96] tracking-[-.035em] text-[#fff9e8] sm:text-6xl">Tenha o Método CASAR+ para tomar decisões mais inteligentes durante o planejamento do seu casamento.</h2>
            ...
            <p className="mt-4 text-sm font-semibold text-[#ead9ad]">Esta é uma condição especial de lançamento do Método CASAR+.</p>
          </div>
          <article className="offer-livret relative mx-auto mt-12 max-w-xl overflow-hidden border border-[#d8b76d]/55 bg-[#fffdf7] shadow-[0_28px_65px_rgba(0,0,0,.22)]">
            ...
            <div className="relative z-10 p-7 sm:p-9">
              ...
              <div className="mt-6 flex flex-wrap items-end gap-x-4 gap-y-2"><span className="text-sm font-bold text-[#808274]">De <span className="line-through">R$ 997,00</span></span><span className="rounded-full bg-[#e5d0a3] px-3 py-1 text-[0.61rem] font-extrabold tracking-wide text-[#675027] uppercase">Por R$ 497,00</span></div>
              ...
              <p className="mt-3 text-sm leading-6 text-[#66705d]">As condições de pagamento disponíveis serão apresentadas no checkout oficial.</p>
              <div className="gold-rule my-7" />
              <p className="font-display text-2xl leading-7 text-[#3c4b34]">Você não está comprando apenas aulas.</p>
              <p className="mt-2 text-sm leading-6 text-[#5d6855]">Está adquirindo um conjunto de ferramentas para tomar decisões melhores em um casamento que pode envolver dezenas de milhares de reais.</p>
              ...
              <div className="mt-7 flex gap-3 border-y border-[#e3d8c0] bg-[#faf3e2] p-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#cba969] text-[#9b7132]"><ShieldCheck className="h-4 w-4" /></span><div><p className="text-[0.62rem] font-extrabold tracking-[.14em] text-[#957139] uppercase">Garantia de 7 dias</p><p className="mt-1 text-sm leading-5 text-[#5d6855]">Você tem 7 dias para conhecer o Método CASAR+ por dentro. Se entender que não faz sentido, basta solicitar o cancelamento dentro do prazo da garantia.</p></div></div>
